...
let timeValue = 15;
let que_count = 1;
let que_numb = getRandomInt(questions.length);
let userScore = 0;
let counter;
let counterLine;
...
function showQuestion() {
  ...
  for (i = 0; i < option.length; i++) {
    // option[i].onclick = optionSelected(option[i]);
...
function optionSelected(answer) {
  ...
  let correcAns = questions[que_numb].answer; //getting correct answer from array
...
function queCounter(index){
    ...
    let totalQueCounTag = '<span><p>'+ index +'</p> of <p>'+ total_ques +'</p> Questions</span>';
